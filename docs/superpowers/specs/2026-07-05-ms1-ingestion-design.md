...
- **Denominator + resolution evidence (Codex SA-003 — the gate must not be passable on trivially few matches or `grain = none` rows):** the corpus holds **150–200 titles spanning all 5 sources**, and the precision claim requires **≥ 100 rung-0–2 auto-accepted decisions** in its denominator (floor provisional-tunable; a smaller denominator fails the evaluation with "insufficient corpus", never "pass"). MS-1 close additionally requires the **FR-003 acceptance case green as a resolver-driven test** (a recert and a new listing of the same drive → one `product_model`, two `product_variant`s — upgraded from MS-0's schema-shape version at MS-1b) and the evaluation report stating **per-source model-grain-or-better coverage** (reported for MS-2's ≥ 80% expectation; not an MS-1 gate).
