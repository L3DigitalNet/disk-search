...
resolved-questions.md gap #1 is resolved to single-account session login, and resolved question RQ2 (public URL vs Tailscale-only) is **public URL required**. Confirmed when the schema includes the `users` stub and login uses Argon2id with hardened cookie flags.
