...
- **Bad (mitigable)** — backup coverage is a **hardcoded allowlist, not auto-discovery**: provisioning **must** wire the CT's data paths into `backup-restic.sh` and its DB into `backup-dumps.sh` (+ mirror config per the homelab "Maintenance" checklist), or the CT is silently unprotected. A fail-loud guard catches a _disappearing_ declared path, but not a _never-added_ one. **Because the DB uses TimescaleDB (ADR 0007), a plain `pg_dump` allowlist entry is not sufficient** — the dump/restore must be TimescaleDB-aware (`timescaledb_pre_restore()`/`post_restore()`; native-compression state not preserved), or in-CT physical backup must be added (resolved-questions.md OQ3).
- **Neutral** — the inherited database RPO is **≤1 h with no PITR** (hourly logical dumps). Whether that is sufficient for the price-history moat, or pgBackRest + WAL archiving must be layered inside the CT, is tracked as resolved-questions.md OQ3 — independent of this container-vs-VM decision.
...
- **Downstream consequences:** OQ3 (DB-RPO acceptance) **resolved 2026-07-04** — ≤1 h RPO accepted for v1, TimescaleDB-aware logical dumps (resolved-questions.md); **OQ4 settled 2026-07-03 → own Postgres inside the hw-radar CT** (self-contained); gap #2 (secrets via local OpenBao Agent).
