...
**The database lives on a container Postgres.** Whether that is a Postgres inside the disk-search CT (self-contained) or the shared datastores CT (centralized, already in the dump pipeline) is a deferred sub-decision (gap-analysis open-question #10); both are compatible with this ADR.
...
- **Bad (accepted trade-off)** — a CT has **no per-container vTPM** (containers share the host kernel), so `systemd-creds --with-key=tpm2` hardware-bound secret storage is unavailable. Secrets resolve via a **local OpenBao Agent** instead (gap-analysis gap #2). Acceptable because the app is designed to hold no in-app secrets and the static-secret set is small.
- **Bad (mitigable)** — backup coverage is a **hardcoded allowlist, not auto-discovery**: provisioning **must** wire the CT's data paths into `backup-restic.sh` and its DB into `backup-dumps.sh` (+ mirror config per the homelab "Maintenance" checklist), or the CT is silently unprotected. A fail-loud guard catches a _disappearing_ declared path, but not a _never-added_ one. **Because the DB uses TimescaleDB (ADR 0007), a plain `pg_dump` allowlist entry is not sufficient** — the dump/restore must be TimescaleDB-aware (`timescaledb_pre_restore()`/`post_restore()`; native-compression state not preserved), or in-CT physical backup must be added (open-Q #9).
- **Neutral** — the inherited database RPO is **≤1 h with no PITR** (hourly logical dumps). Whether that is sufficient for the price-history moat, or pgBackRest + WAL archiving must be layered inside the CT, is tracked as gap-analysis open-question #9 — independent of this container-vs-VM decision.
...
The spec's Server Configuration section states "Dedicated LXC container … (not a VM)" and carries a dedicated Backup & Monitoring bullet; gap-analysis open-question #8 is marked **DECIDED: dedicated LXC container**, with the vTPM consequence reflected in gap #2. Provisioning-time confirmation: the CT appears in `pct list` (→ auto-monitored) and its paths/DB appear in `backup-restic.sh`/`backup-dumps.sh` (→ backed up).

## More Information

- **Findings that forced the decision:** gap-analysis [`#5` (backup) and `#6` (observability)](../gap-analysis.md) "Live-state findings" blocks, and open-question **#8**.
- **Downstream consequences tracked as open questions:** #9 (DB-RPO acceptance), #10 (own-PG-in-CT vs shared datastores CT), and gap #2 (secrets via local OpenBao Agent).
