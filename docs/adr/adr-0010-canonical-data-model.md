...
- **Good** — retention/PII/licensing are enforced *in the schema* (`retention_class`/`expires_at`, no image bytes), so the acquisition-legal posture is structural, not conventional.
- **Bad (accepted complexity)** — more grains than a naive catalog: most queries join `product_model → drive_spec` and `listing → product_variant`. Justified only because the identity table is the one thing too costly to reshape later; every *cheap-to-add* concern (RAM specs, extra scoring) is deferred.
