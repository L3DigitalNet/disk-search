...
**Build & release mechanics:** build the venv **on the CT** (`uv sync --frozen`) to avoid arch/path skew; run migrations **before** restart using **expand/contract** (backward-compatible with the still-running old code). **Service topology (systemd):** a **web** unit (gunicorn + uvicorn workers, `ExecReload=kill -HUP $MAINPID`) and **worker** unit(s) (`Restart=on-failure`) under a **dedicated non-root user** with `ProtectSystem=strict`/`NoNewPrivileges`; periodic scrapes run under a single **in-process scheduler** (APScheduler 3.11.x) inside a systemd-supervised **poller** unit — systemd supervises the _process_ (restart-on-failure, resource limits, journal logging), it does not schedule each scrape; `systemd` timers are kept only for genuinely independent, stateless jobs (e.g. nightly `VACUUM`, backup verification). **Amended 2026-07-04 by [ADR 0012](adr-0012-orchestration-apscheduler.md)** — the original "systemd timers for scrapes" wording predated the finding that these scrapes share admission-budget and circuit-breaker state and therefore belong in one process. Avoid `Type=notify` — plain gunicorn never calls `sd_notify()` and the unit would time out.
