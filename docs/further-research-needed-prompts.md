...
| 14 | AgentMail deliverability ([OQ13](resolved-questions.md#oq13--notification-transport--deliverability-agentmail-vs-transactional-provider)) | ✅ | [email-path](research/choosing-an-outbound-email-path-for-a-low-volume-alerting-system.md) | **Ratified (OQ13 / [ADR-0013](adr/adr-0013-notification-transport-m365-graph.md)): reuse M365 Graph primary + AgentMail free fallback; Postmark/SES retained only as a paid-upgrade path** (supersedes this report's original "Postmark primary" recommendation). AgentMail is _not_ disqualified on DKIM — the factor is its thinner transactional-deliverability track record; branded custom-domain sending needs its $20/mo tier (free tier is `@agentmail.to` only) |
...
> **✅ Answered** — [`opinionated-core-stack-recommendations`](research/opinionated-core-stack-recommendations-for-a-python-drive-price-monitor.md). Resolves all three `_TBD_` markers: **Django** (server-rendered templates + HTMX, over FastAPI/Flask) for a CRUD/dashboard/auth/ingestion app, **PostgreSQL 18** (JSONB + full-text; **no** TimescaleDB and no partitioning until the price-history table demands it), and **uv** for env/dependency management. **⚠️ Datastore call later revised** (this report judged TimescaleDB unnecessary at hobby scale): the price-history side *does* adopt **PostgreSQL + TimescaleDB** ([ADR-0007](adr/adr-0007-datastore-postgresql-timescaledb.md) / RQ6, per [`database-architecture`](research/database-architecture.md)); the Django + uv picks stand. **Residual:** none — reconciliation into the spec and CLAUDE.md is complete (the old "FastAPI or Django (undecided)" note is resolved).
