...
- 2026-07-06: MS-1e validation-corpus + ADR-0019 ratification **design spec written and Codex-converged** (4 adversarial passes, final verdict "no significant findings remain"): `docs/superpowers/specs/2026-07-06-ms1e-validation-corpus-ratification-design.md`. It specifies a deterministic evaluation harness (Approach A — real `CatalogResolver` over a seeded test DB), the `harvest_corpus` command, a grain-shaped natural-key corpus (`manufacturer_key` + loader-normalized family/model + `ProductVariant` enums), a composite `ms1_ratification_gate` (precision + per-source family floor + rung-0 regression), and a skip-guarded ratification test. The implementation plan (`writing-plans`), live harvest, label audit, and ADR-0019 flip are deferred to follow-up sessions.
